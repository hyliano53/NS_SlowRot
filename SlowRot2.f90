...
! Constant variables
double precision pi,mevtokmm2,kmpersunsmass,k,NuR,cspeed,xi0,p2,xi2,rp,re,ellip,deltam,Jtot
...
double precision Maux,Edensaux2,Paux,drhodpaux,dmdraux,jinterpol,derjinterpol,aaux,waux,dnudraux,Omega
...
open(unit=60,file='resultados.dat')
...
enddo
 ...
 Omega=(w(len)+r(len)*a(len)/3)

!This loop reescales the agular velocity and its derivative. The number that appears is the angular velocity desired.
do i=1,len
      w(i)=w(i)*5000d0/(Omega*cspeed)
      a(i)=a(i)*5000d0/(Omega*cspeed)
enddo

Omega=(w(len)+r(len)*a(len)/3)

print*, 'Omega (s^-1):', Omega*cspeed
print*, 'Omega (ms^-1):', Omega*cspeed/1000d0
print*, r(len), w(len), Omega*r(len)
...
write(60,*) (M(len)+deltam)/kmpersunsmass, Jtot/(Omega*kmpersunsmass), ellip, Jtot
